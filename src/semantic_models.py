...
class SemanticTrainer():
    def __init__(self, USE_CUDA=True, max_iter=200, lr=1e-3):
        ...
        for f in files:
            self.features[f[:-4]] = torch.from_numpy(
                np.load(cfg.TRAIN_SEMANTIC_FEATURES_PATH + f))
            ...
            for aid, papers in labeled_paper.items():
                ...
                for i in range(cur_n):
                    ...
                    for i_pos in idx_pos:
                        ...
                        pid_pos = pids[i_pos]
                        pid_neg = self.gen_neg_pid(
                            pids, paper_by_author[author])
                        # print(pid)
                        # print(np.array([all_paper.index(pid), all_paper.index(pid_pos), all_paper.index(pid_neg)]))
                        triplets = np.concatenate((triplets, np.array(
                            [[all_paper.index(pid), all_paper.index(pid_pos), all_paper.index(pid_neg)]])))
                        n_sample_triplets += 1
    ...
    def experiment(self, epoch, datapath=cfg.TRAIN_SEMANTIC_FEATURES_PATH, exp_author='xu_shen'):
        self.model.load_state_dict(torch.load(
            '../model/semantic_model' + str(epoch) + '.pkl'))
        ...
        for aid, papers in exp_author.items():
            ...
            j += 1
        if str(epoch) == '1':
            self.plot_embedding_2d(
                arr_tsne, target, "t-SNE 2D Initial Embedding")
        self.plot_embedding_2d(
            arr_tsne, target, "t-SNE 2D Trained " + str(epoch) + ' Epoch Embedding')
